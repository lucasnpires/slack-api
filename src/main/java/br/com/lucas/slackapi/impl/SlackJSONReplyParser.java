...
import br.com.lucas.slackapi.replies.SlackReplyImpl;
...
class SlackJSONReplyParser {
	static ParsedSlackReply decode(JsonObject obj, SlackSession session) {
		...
		if (isMessageReply(obj)) {
			...
			return new SlackMessageReply(ok, error, GsonHelper.getLongOrDefaultValue(obj.get("reply_to"), -1l),
					timestamp);
		}

		if (isEmojiReply(obj)) {
			String timestamp = GsonHelper.getStringOrNull(obj.get("cache_ts"));
			return new EmojiSlackReply(ok, error,
					SlackJSONMessageParser.extractEmojisFromMessageJSON(obj.get("emoji").getAsJsonObject()), timestamp);
		}
	...
	private static SlackChannelReply buildSlackChannelReply(Boolean ok, String error, JsonObject obj,
